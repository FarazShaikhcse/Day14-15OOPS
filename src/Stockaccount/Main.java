...
/**
 * @author faraz 
 * this class performs all the processing
 */
public class Main {
	...
	private static void buyStocks() {
		...
		while (amount > (long) selectedStock.get("noOfShares") || amount <= 0) {
			System.out.println(amount + " shares not available, Please enter lesser no of shares to buy");
	...
	private static void sellStocks() {
		...
		while (amount > (long) selectedStock.getNoOfShares() || amount <= 0) {
			System.out.println("You don't have " + amount + " shares to sell, please enter a lower amount");
			amount = in.nextInt();
		}
